...
class SalesAction extends CommonAction {
  ...
  public function mxj(){
    ...
    //明星阵容
    $this->start = M('ads')->where('cat_id=15')->order('sort_order ASC, add_time DESC')->limit(9)->select();

    //明星品质
    $this->start_quality = M('ads')->where('cat_id=16')->order('sort_order ASC, add_time DESC')->limit(7)->select();
