...
class GoodsAction extends CommonAction {
	...
	public function add(){
        ...
        foreach ($cat_list as $key => $value) {
            $cat_list[$key]['list'] = M('goods_ground')->where('cat_id='.$value['cat_id'])->select();

        }

        $this->goods_color = M('goods_color')->select();
        $this->goods_mesa=M('goods_mesa')->select();
	...
	public function edit(){
		...
		$this->check_cat = explode(',', $info['cat_id']);
		$this->check_color = explode(',', $info['color']);
		$this->check_mesa = explode(',', $info['mesa']);

		$scat_id = explode(',', $cat_id);
		foreach ($scat_id as $key => $value) {
			$goods_cat_edit[] = M('goodscat')->where('cat_id='.$value)->find();
		}
		foreach ($goods_cat_edit as $k => $v) {

			$property = M('goods_ground')->where(array('cat_id'=>$v['cat_id'],'parent_id'=>0))->select();

			foreach ($property as $key => $value) {

				$property[$key]['list']=M('goods_ground')->where(array('parent_id'=>$value['id']))->select();
				
			}
			//$scat_id[$k] = $cat_name_find['cat_name'];

			$goods_cat_edit[$k]['three'] = $property;
		}
		$mucai = explode(',', $info['mucai']);
		foreach ($mucai as $key => $value) {
			$info['mucai_list'][] = $value;
		}

		$this->goods_cat_edit = $goods_cat_edit;
		$this->goods_color = M('goods_color')->select();
        $this->goods_mesa=M('goods_mesa')->select();
	...
	public function insert(){
		...
		$data['mesa']  = implode(',', $_POST['mesa']);
		$data['mucai']  = implode(',', $_POST['mucai']);
