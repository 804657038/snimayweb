...
class ArticleAction extends CommonAction {
    ...
    $cat_config=C('cat_style');
    $cat_style = M('articlecat')->where("cat_id=$cat_id")->getField('cat_style');
    $tpm=$cat_config[$cat_style][0];

		$this->assign('cat_id', $cat_id);
		$site_config = include WEB_ROOT . 'Common/systemConfig.php';
		$site_info = $site_config['SITE_INFO'];
		...
		$tag_list=array();
		str_replace("，",",",$site_info['tag_list']);
		$tag_list = explode(',', $site_info['tag_list']);
	...
	public function edit(){
		...
		$info['list'] = explode(',', $info['tip']);

		$this->assign("cat_select", article_cat_list(0,$info['cat_id']));
		
		$this->assign("info", 	$info);

		$this->assign("attr",unserialize($info['attr']));
        ...
        $cat_config=C('cat_style');
        $cat_style = M('articlecat')->where("cat_id=".$info['cat_id'])->getField('cat_style');
        $tpm=$cat_config[$cat_style][1];
        $cat_en_name = M('articlecat')->where("cat_id=".$info['cat_id'])->getField('cat_en_name');
        if($cat_en_name=="press"){
            $press=M('article')->where("article_id=".$article_id)->find();
            $press_img=unserialize($press['original_img']);
            $this->assign('press',$press);
            $this->assign('press_img',$press_img);
            $tpm="edit_press";
        }
		$this->display($tpm);
	}
	...
	public function insert(){
		...
		$data['f_time']     = $_POST['f_time']? strtotime($_POST['f_time']) : time();
		$data['attr']		= serialize($data['attr']);//属性
		$data['tip']        = implode(',', $_POST['tip']);

    if($_POST['video']){
      $str = array_filter(explode('\\', $_POST['video']));
      foreach ($str as $value) {
        if(stripos($value, '.mp4') !== false || stripos($value, '.swf') !== false){
          $data['video'] = $value;
          break;
        }
      }
		...
		if($_FILES['article_img1']['error']===0){
			$originalPath='Uploads/article/original_img/'.time().rand(10,99).'.'.pathinfo($_FILES['article_img1']['name'],PATHINFO_EXTENSION);
			move_uploaded_file($_FILES['article_img1']['tmp_name'], $originalPath);
			$data['original_img1']  = $originalPath;
		}
    ...
    if(!empty($_POST['original_img'])){
      $data['original_img']=addslashes($_POST['original_img']);
    }
    if(!empty($_POST['attr'])){
      $data['attr']=serialize($_POST['attr']);
    }
		$M_Article = M("article");
	...
	public function update(){
		...
		$data['attr']		= serialize($data['attr']);//属性
		$data['tip']        = implode(',', $_POST['tip']);
		$data['web_title']		= $_POST['web_title'];//属性
    ...
    if($_POST['video']){
      ...
      foreach ($str as $value) {
		...
		if(!empty($_FILES['article_img1']['tmp_name'])){
			@unlink($oldRow['thumb_img1']);
			@unlink($oldRow['original_img1']);
			$originalPath='Uploads/article/original_img/'.time().rand(10,99).'.'.pathinfo($_FILES['article_img1']['name'],PATHINFO_EXTENSION);
			move_uploaded_file($_FILES['article_img1']['tmp_name'], $originalPath);
			$data['original_img1']  = $originalPath;
		}
        ...
        if(!empty($_POST['original_img'])){
            if($_POST['original_img']!=$oldRow['original_img']){
                @unlink($oldRow['original_img']);
            }
    ...
    //产品相册
	public function addArticleimg(){
		$article_id = $_GET['id']+0;
		$this->detail = M('article')->where('article_id='.$article_id)->find();
		$this->articleimg_list = M('articleimg')->where('article_id='.$article_id)->order('sort_order asc,id desc')->select();
		$this->display('articleimg');
	}

	
	public function updateArticleimg(){
		$article_id = $_POST['id']+0;

		$new_img_sort = $_POST['new_img_sort'];
		$old_img_id	  = $_POST['old_img_id'];
		foreach($new_img_sort as $k=>$v){
			M('articleimg')->where('id='.$old_img_id[$k])->save(array('sort_order'=>$v));
		}
	...
	public function delimg(){
		$id = $_GET['id']+0;
		$oldrow = M('articleimg')->where('id='.$id)->find();
		M('articleimg')->where('id='.$id)->delete();
		@unlink($oldrow['original_img']);
		@unlink($oldrow['thumb_img']);
	}
    ...
    public function save_press(){
          if($_POST){
              $data=array();
              $data['title']=!empty($_POST['title'])?addslashes($_POST['title']):$this->error('标题错误');
              $data['cat_id']=!empty($_POST['cat_id'])?addslashes($_POST['cat_id']):$this->error('请选择分类');
              $data['sort_order']=addslashes($_POST['sort_order']);
              $data['add_time']=!empty($_POST['add_time'])?addslashes(strtotime($_POST['add_time'])):time();

              $images=array();
              if($_FILES){

                  if($_FILES['firstimg']['error']===0){
                      $originalPath='Uploads/article/original_img/'.time().'.'.pathinfo($_FILES['article_img']['name'],PATHINFO_EXTENSION);
                      move_uploaded_file($_FILES['firstimg']['tmp_name'], $originalPath);
                      $data['original_img1']  = $originalPath;
                  }
                  unset($_FILES['firstimg']);

                  $num=0;
                  foreach ($_FILES as $k=>$v){
                      $originalPath='Uploads/article/original_img/'.time().'_'.$k.'.'.pathinfo($v['name'],PATHINFO_EXTENSION);
                      move_uploaded_file($v['tmp_name'], $originalPath);
                      $images[$num]=$originalPath;
                      $num++;
                  }
              }
              $press=is_array($_POST['press'])?$_POST['press']:$this->error('请上传报刊图片');
              foreach ($press as $k=>$v){
                  $press[$k]['url']=$images[$k];
              }
              $data['original_img']=serialize($press);
              $data['add_time']=time();
              $M_Article = M("article");
              $insertId=$M_Article->data($data)->add();
              if($insertId){
    ...
    public function update_press(){
        if($_POST){
            ...
            if($_FILES){
                ...
                $data['original_img1']  = $data['firstimg'];
            }
            $data['add_time']=!empty($_POST['add_time'])?addslashes(strtotime($_POST['add_time'])):time();
            $data['original_img']=serialize($_POST['press']);
            $M_Article = M("article");
            $M_Article->where(array('article_id'=>$data['article_id']))->save($data);
    ...
    public function upload_img(){
        if($_FILES['photo']['error']==0){
            $k=rand(1000,9999);
            $originalPath='Uploads/article/original_img/'.time().'_'.$k.'.'.pathinfo($_FILES['photo']['name'],PATHINFO_EXTENSION);
