...
class ProductAction extends CommonAction {
    ...
    public function index($cat_id=0) {
        //网站标题 关键字 描述
        //$this->site_title = '产品体检_' .$this->site_info['title'];
        ...
        if($cat_info['keywords']){
            ...
            $this->site_keywords = "定制衣柜，家具定制，全屋定制家具，定制家具，整体衣柜，诗尼曼全屋定制
";
        }
        ...
        if($cat_info['cat_desc']){
            ...
            $this->site_description = "诗尼曼产品体验中心，提供卧房，厨房，客厅，书房，餐厅，儿童房的3D立体效果体验，让业主真切感受自家安装效果，同时提供免费上门量尺服务，让定制家具体验更省心。
";
        }
        ...
        $limit = $_REQUEST['limit']? $_REQUEST['limit']+0 : 9;
...
//                $ground_w="cat_id=".$value['cat_id']." AND parent_id=0";
        ...
        $cat_id2_name=M('goods_ground')->where('id='.$cat_id2)->getField("title");
        $cat_id3_name=M('goods_ground')->where('id='.$cat_id3)->getField("title");
        $cat_id4_name=M('goods_ground')->where('id='.$cat_id4)->getField("title");
        if(!empty($cat_id2)&&$one_cat_res['chi_id']==1){
            $grounds=array();
            $ground= M('goods_ground')->where(array('parent_id'=>$cat_id2))->field('id')->select();
            foreach($ground as $k=>$v){
                $grounds[$k]=$v['id'];
            }
            $this->arr_cat_id2=array("cat_id"=>$cat_id2,"title"=>$cat_id2_name);
            if(in_array($cat_id3,$grounds)){
                $this->arr_cat_id3=array("cat_id"=>$cat_id3,"title"=>$cat_id3_name);
            }else{
                $this->arr_cat_id3=null;
            }
            if(in_array($cat_id4,$grounds)){
                $this->arr_cat_id4=array("cat_id"=>$cat_id4,"title"=>$cat_id4_name);
            }else{
                $this->arr_cat_id4=null;
            }
        }else{
    ...
    public function detail(){
        ...
        $cat_list = M('goodscat')->where('parent_id = 0')->order('cat_id asc')->select();
        foreach ($cat_list as $key => $value) {
            $cat_list[$key]['list'] = M('goods_ground')->where('cat_id='.$value['cat_id'].' AND parent_id=0')->order('sort_order asc')->select();
        }
        $this->cat_list = $cat_list;
        //面包屑导航
        ...
        /*if($detail['web_title']){
            $this->site_title = $cat_info['web_title'];
        }else{
            $this->site_title = $this->site_info['title'];
        }*/
