...
class VisitAction extends CommonAction {
  ...
  public function city_detail($cat_id=61){
    ...
    if($select == 'district'){
      ...
      foreach ($town_list as $key => $value) {
        ...
        if($data_list){
          foreach ($data_list as $u => $d) {
            ...
            if($d['schedule'] == 11 || $d['schedule'] == 12 || $d['schedule'] == 14){
              $town_list[$key]['status'] = '4';
              continue;
            }
          }
        }else{
      ...
      $this->u_here = '<a href="'.U('Visit/index').'">首页</a> &nbsp; > &nbsp; <a href="'.U('Visit/city_detail', array('province_id'=>$province_info['province_id'])).'">'.$province_info['province_name']."</a> &nbsp; > &nbsp; <a href='".U('Visit/city_detail', array('city_id'=>$city_info['city_id']))."'>".$city_info['city_name']."</a> &nbsp; > &nbsp; <a href='".U('Visit/detail', array('district_id'=>$district_id))."'><em>".$district_info['country_name']."</em></a>";
      ...
      $province_info = M('region_province')->where('province_id='.$city_info['province_id'])->find();
      $this->u_here = '<a href="'.U('Visit/index').'">首页</a> &nbsp; > &nbsp; <a href="'.U('Visit/city_detail', array('province_id'=>$province_info['province_id'])).'">'.$province_info['province_name']."</a> &nbsp; > &nbsp; <a href='".U('Visit/city_detail', array('city_id'=>$city_info['city_id']))."'><em>".$city_info['city_name']."</em></a>";
      ...
      $this->u_here = '<a href="'.U('Visit/index').'">首页</a> &nbsp; > &nbsp; <a href="'.U('Visit/city_detail', array('province_id'=>$province_info['province_id'])).'"><em>'.$province_info['province_name']."</em></a>";
  ...
  public function detail($cat_id=61){
    ...
    if($town_id){
      ...
      $city_info = M('region_city')->where('city_id='.$district_info['city_id'])->find();
      $province_info = M('region_province')->where('province_id='.$city_info['province_id'])->find();
      $this->u_here = '<a href="'.U('Visit/index').'">首页</a> &nbsp; > &nbsp; <a href="'.U('Visit/city_detail', array('province_id'=>$province_info['province_id'])).'">'.$province_info['province_name']."</a> &nbsp; > &nbsp; <a href='".U('Visit/city_detail', array('city_id'=>$city_info['city_id']))."'>".$city_info['city_name']."</a> &nbsp; > &nbsp; <a href='".U('Visit/detail', array('district_id'=>$district_info['country_id']))."'>".$district_info['country_name']."</a>";
...
// echo '<pre>';print_r($district_info);echo '</pre>';
  ...
  public function search($cat_id=61){
    ...
    if($province){
      @header('Location:'.U('Visit/city_detail', array('province_id'=>$province['province_id'])));
      exit();
    }
    $city = M('region_city')->where('city_name = "'.$keyword.'"')->find();
    if($city){
      @header('Location:'.U('Visit/city_detail', array('city_id'=>$city['city_id'])));
      exit();
    }
    $district = M('region_country')->where('country_name = "'.$keyword.'"')->find();
    if($district){
      @header('Location:'.U('Visit/detail', array('district_id'=>$district['country_id'])));
      exit();
    }
    $town = M('region_town')->where('town_name = "'.$keyword.'"')->find();
    if($town){
      @header('Location:'.U('Visit/detail', array('town_id'=>$town['town_id'])));
      exit();
    }
